 ...
 *
 * Serverless instances are short-lived and not shared, so an in-memory bucket
 * only catches bursts that happen to land on the same warm instance. The
 * database check is the authoritative one. We do both: the memory check is
 * free and stops the common case early, the DB check is correct.
 */
...
/**
 * Returns { ok: true } or { ok: false, status, message, retryAfter }.
...
export async function checkRunLimit(userKey, ip) {
  ...
  try {
    ...
    // Never block a learner because the limiter itself failed.
