...
import { applyCors, requireIdentity, sendError, isGuestKey } from './_auth.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Guest -> account migration. The destination is the verified auth uid, and
    // the source must be a guest key (never another account), so this cannot be
    // used to pull a real user's progress.
    const migrateFrom = String(req.query.migrate_from || '');
    if (
      !isGuest &&
      isGuestKey(migrateFrom) &&
      migrateFrom !== userKey &&
      (profile.xp || 0) === 0
    ) {
        ...
        if (old) {
          const { data: upd } = await supabase
            ...
            .update({
              ...
              streak: old.streak || 0,
              hearts: old.hearts ?? 5,
              lessons_done: old.lessons_done || 0,
              last_active: old.last_active || today(),
            })
            .eq('user_key', userKey)
            .select()
            .single();
    ...
    // Never echo the email back to the client beyond the owner's own record.
    return res.status(200).json({
      ...
      achievements: (achievements || []).map((a) => ({
        ...
        earned_at: earnedMap.get(a.slug) || null,
      })),
    });
  } catch (err) {
    return sendError(res, err, 'Could not load your progress.');
  }
}
