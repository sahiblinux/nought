import { createSupabase, json, optionsResponse, requireIdentity, errorResponse, getQuery, getBody } from './_lib.js';
...
async function evaluateAchievements(supabase, userKey, profile) {
  ...
  const tests = {
    ...
    polyglot: langs.size >= 3, pentaglot: langs.size >= 5,
    xp_500: (profile.xp || 0) >= 500, xp_1500: (profile.xp || 0) >= 1500,
    flawless, quiz_ace: quizAce, module_master: moduleDone,
    streak_3: (profile.streak || 0) >= 3, streak_7: (profile.streak || 0) >= 7,
    rustacean: (perLang.rust || 0) >= 3, close_to_metal: (perLang.c || 0) >= 3,
    snake_charmer: (perLang.python || 0) >= 3, jvm_pilot: (perLang.java || 0) >= 3,
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    if (query.language) q.eq('language_slug', String(query.language));
    const { data, error } = await q;
    if (error) throw error;
    return json(data || []);
  } catch (err) {
    return errorResponse(err, 'Could not save your progress.');
  }
}
...
export async function onRequestPost(context) {
  const supabase = createSupabase(context.env);
  try {
    const { userKey, displayName } = await requireIdentity(context.request, supabase);
    ...
    if (!profile) {
      const { data: created, error } = await supabase.from('profiles').insert({
        ...
        xp: 0, streak: 0, hearts: 5, lessons_done: 0,
        last_active: today(), created_at: new Date().toISOString(),
      }).select().single();
      if (error) throw error;
      profile = created;
    }
    ...
    if (heartLoss && !lessonId) {
      ...
      return json({ profile: upd || profile, progress: null, newAchievements: [] });
    }

    if (!lessonId) return json({ error: 'lesson_id required' }, 400);

    const { data: lesson } = await supabase.from('lessons')
      .select('id,xp,language_slug,kind,expected_output,check_mode').eq('id', lessonId).maybeSingle();
    ...
    const { data: existing } = await supabase.from('progress')
      .select('*').eq('user_key', userKey).eq('lesson_id', lessonId).maybeSingle();
    ...
    const row = {
      user_key: userKey, lesson_id: lessonId, language_slug: lesson.language_slug,
      status, attempts, best_code: passed || !existing?.best_code ? code : existing.best_code,
      xp_earned: xpEarned, completed_at: status === 'completed' ? existing?.completed_at || nowIso : null,
      updated_at: nowIso,
    };

    let progressRow;
    if (existing) {
      const { data, error } = await supabase.from('progress').update(row).eq('id', existing.id).eq('user_key', userKey).select().single();
      if (error) throw error;
      progressRow = data;
    } else {
    ...
    await supabase.from('submissions').insert({
      user_key: userKey, lesson_id: lessonId, language_slug: lesson.language_slug,
      code, output, passed, created_at: nowIso,
    });
    ...
    let fresh = [];
    if (newlyCompleted) {
      const result = await evaluateAchievements(supabase, userKey, profileOut);
      fresh = result.fresh;
      if (result.bonusXp > 0) {
        const { data: bumped } = await supabase.from('profiles').update({ xp: (profileOut.xp || 0) + result.bonusXp }).eq('user_key', userKey).select().single();
        if (bumped) profileOut = bumped;
      }
    }

    return json({ profile: profileOut, progress: progressRow, newAchievements: fresh, passed, xpGained: newlyCompleted ? lesson.xp || 20 : 0 });
  } catch (err) {
    return errorResponse(err, 'Could not save your progress.');
  }
}
